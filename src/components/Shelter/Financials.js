...
const ShelterFinancials = (props) => {
    ...
    <div className = "hero">
        ...
        <Grid item xs = {9}>
            {orders ? <BasicTable columns = {['Total Value', 'Store', 'Quantity', 'Order Date', 'Status', 'View Details']} rows = {orders} totalValue = {totalValue} usedIn = "Financials" /> : "Loading..."}
        </Grid>
        <Grid item xs = {3}>
            <div className = "store_information" style = {{height: '95%'}}>
                <div style = {{margin : '20% auto', fontSize : '150%'}}>You've received a total value of <span style = {{fontWeight : "bold"}}>${totalValue}</span>  through donations!</div>
            </div>
        </Grid>
        </Grid>
